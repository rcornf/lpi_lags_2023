...
## Supplementary analyses - Ecological subsetting
##%%%%%%%%%%
...
m_fit_wrp <- function(df, cc_cl, luc_col, dp_min){
    print("Herb")
    herb <- m_fit(subset(df, Herb_pcnt>66),
                  T, cc_cl, luc_col, dp_min)
    herb$ecol_sub <- "herb"
    
    print("Carn")
    carn <- m_fit(subset(df, Carn_pcnt>66),
                  T, cc_cl, luc_col, dp_min)
    carn$ecol_sub <- "carn"
    
    # Add in tropical/temperate
    print("Trop")
    # trop <- m_fit(subset(df, Latitude < 23.5 & Latitude > -23.5),
    #               T, cc_cl, luc_col, dp_min)
    trop <- m_fit(subset(df, trop_temp == "trop"),
                  T, cc_cl, luc_col, dp_min)
    trop$ecol_sub <- "trop"
    
    print("Tmpr")
    # tmpr <- m_fit(subset(df, Latitude > 23.5 | Latitude < -23.5),
    #               T, cc_cl, luc_col, dp_min)
    tmpr <- m_fit(subset(df, trop_temp == "tmpr"),
                  T, cc_cl, luc_col, dp_min)
    tmpr$ecol_sub <- "tmpr"
    
    # and central/extreme
    # change, use thirds...
    lwr <- quantile(df$lambda, 1/3)
    upr <- quantile(df$lambda, 2/3)
    
    # gamma for x...
    print("Xneg")
    xneg <- gamma_m_fit(subset(df, lambda <=  lwr),
                        T, T, cc_cl, luc_col, dp_min)
    xneg$ecol_sub <- "xneg"
    
    print("Cntr")
    cntr <- m_fit(subset(df, lambda <= upr & lambda > lwr),
                  T, cc_cl, luc_col, dp_min)
    cntr$ecol_sub <- "cntr"
    
    print("Xpos")
    xpos <- gamma_m_fit(subset(df, lambda > upr),
                        T, F, cc_cl, luc_col, dp_min)
    xpos$ecol_sub <- "xpos"
...
for (j in 1:nrow(m_spec)){
    # Load rate data
    f = paste0("lpi_lag_res_", as.character(jobid), ".rds")
    fp = paste0("../Results/ecol_models/", f)
    spec <- m_spec[j,]
