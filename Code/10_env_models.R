...
## Supplementary Analyses - testing use of alternative env data
##%%%%%%%%%%
...
m_fit_wrp <- function(df, cc_cl, luc_col, dp_min, spec){
    ...
    # add spec info to results dataframe
...
for (j in 1:nrow(m_spec)){
    ...
    fp = paste0("../Results/env_models/", f)
    ...
    # Fit models (incl CI, ret m), loo and loo_bsr (no CI)
    # Combinations of env data and dpmin
    ccluc_comb <- data.frame(cc_col = c("ipsl", "ipsl", 
                                        "cru4.04", "cru4.04", 
                                        "cru4.04", "cru4.04"),
                             luc_col = c("hyde3.2_crng", "hyde3.2_grz", 
                                         "hyde3.2_crng", "hyde3.2_grz",
                                         "luh2_norng", "luh2_rng"),
                             dp_min = 3,
                             stringsAsFactors = F)
